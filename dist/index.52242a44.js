...
})({"9VauO":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "108422b552242a44";
...
function showWeatherFromFavorite(event) {
    _mainJs.showNow(_serverJs.SERVER.URL_MAIN, event.currentTarget.textContent);
    _mainJs.showDetails(_serverJs.SERVER.URL_MAIN, event.currentTarget.textContent);
    _mainJs.showForecast(_serverJs.SERVER.URL_FORECAST, event.currentTarget.textContent);
...
function showWeatherFromSearch(event) {
    _mainJs.showNow(_serverJs.SERVER.URL_MAIN, UI_ELEMENTS.INPUT.value);
    _mainJs.showDetails(_serverJs.SERVER.URL_MAIN, UI_ELEMENTS.INPUT.value);
    _mainJs.showForecast(_serverJs.SERVER.URL_FORECAST, UI_ELEMENTS.INPUT.value);
    localStorage.setItem('currentCity', UI_ELEMENTS.INPUT.value);
    event.preventDefault();
}
function showWeatherFromStorage(city) {
    _mainJs.showNow(_serverJs.SERVER.URL_MAIN, city);
    _mainJs.showDetails(_serverJs.SERVER.URL_MAIN, city);
    _mainJs.showForecast(_serverJs.SERVER.URL_FORECAST, city);
}
...
if (_storageJs.currentCity) showWeatherFromStorage(_storageJs.currentCity);
else showWeatherFromStorage(UI_ELEMENTS.CITY_NOW.textContent);
 // export {favoriteList};

},{"./main.js":"adjPd","./storage.js":"kS9sU","./server.js":"6lo7H","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"adjPd":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
function getDate(time) {
    ...
    const day = _dateFns.format(date, 'LLLL d');
    return day;
}
async function showNow(serverUrl, cityName) {
    let city = await jsonWeather(serverUrl, cityName);
    try {
        if (!city.name) throw new Error(city.message);
        else {
            _viewJs.UI_ELEMENTS.TEMP_NOW.textContent = `${kelvinToCelsius(city.main.temp)}°`;
            _viewJs.UI_ELEMENTS.CITY_NOW.textContent = city.name;
        // console.log(city)
        }
    } catch (err) {
        alert(err);
...
function showDetails(serverUrl, cityName) {
    jsonWeather(serverUrl, cityName).then((city)=>{
        _viewJs.UI_ELEMENTS.DETAILS_TEMP.textContent = `${kelvinToCelsius(city.main.temp)}°`;
        _viewJs.UI_ELEMENTS.DETAILS_FEELS.textContent = `${kelvinToCelsius(city.main.feels_like)}°`;
        _viewJs.UI_ELEMENTS.DETAILS_WEATHER.textContent = city.weather[0].main;
        _viewJs.UI_ELEMENTS.DETAILS_CITY.textContent = city.name;
...
function showForecast(serverUrl, cityName) {
    jsonWeather(serverUrl, cityName).then((weather)=>{
        ...
        for(let i = 0; i < _viewJs.UI_ELEMENTS.FORECAST_ITEMS.length; i++){
            ...
            _viewJs.UI_ELEMENTS.FORECAST_DATE[i].textContent = city.date;
            _viewJs.UI_ELEMENTS.FORECAST_TIME[i].textContent = city.time;
            _viewJs.UI_ELEMENTS.FORECAST_TEMP[i].firstElementChild.textContent = city.temp;
            _viewJs.UI_ELEMENTS.FORECAST_FEELS[i].firstElementChild.textContent = city.feels;
            _viewJs.UI_ELEMENTS.FORECAST_WEATHER[i].textContent = city.condition;
        }
        console.log(weather);
    });
}
function addFavorite() {
    ...
    try {
        if (_storageJs.favoriteList.has(cityName)) throw new Error('this city is already on the list');
        else {
            _storageJs.favoriteList.add(cityName);
            localStorage.setItem('favorites', JSON.stringify([
                ..._storageJs.favoriteList
            ]));
...
function deleteFavorite(event) {
    ...
    _storageJs.favoriteList.delete(cityName);
    localStorage.setItem('favorites', JSON.stringify([
        ..._storageJs.favoriteList
    ]));
...
async function jsonWeather(serverUrl, cityName) {
    const url = `${serverUrl}?q=${cityName}&cnt=4&appid=${_serverJs.SERVER.API_KEY}`;
    const json = await fetch(url).then((response)=>response.json()
    );
    return json;
} // export {favoriteList};

},{"./view.js":"de63B","./storage.js":"kS9sU","./server.js":"6lo7H","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","date-fns":"9yHCA"}],"kS9sU":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
...
function showStorage() {
    // console.log(favoriteList);
    if (favoriteList) favoriteList.forEach((item)=>{
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["9VauO","de63B"], "de63B", "parcelRequirec6ff")

//# sourceMappingURL=index.52242a44.js.map
